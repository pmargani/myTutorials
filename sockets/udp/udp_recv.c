...
void byte_swap_long(unsigned long *d) {
    unsigned long tmp;
    char *ptr1, *ptr2;
    ptr1 = (char *)d;
    ptr2 = (char *)&tmp + 3;
...
int main(int argc, char *argv[]) {
        ...
        if (rv > 0) {
            ...
            if (rv==-1) {
                ...
                if (print_all)  {
                    ...
                    /*
                    bufptr += 8;
                    unsigned long num1 = *((unsigned long *)bufptr);
                    unsigned long num2;
                    if (endian) byte_swap_long(&num1);
                    printf("    First num: %20ld\n", num1);
                    for (i=0; i<253; i++) {
                        bufptr += 4;
                        num2 = *((unsigned long *)bufptr);
                        if (endian) byte_swap_long(&num2);
                        printf("    Next  num (%d): %20ld\n", i, num2);         
                    } 
                    */
                    //bufptr += 4*127;
                    //unsigned long num3 = *((unsigned long *)bufptr);
                    //if (endian) byte_swap_long(&num3);
                    //printf("    Last  num: %20ld\n", num3);         
                }
            ...
            if (first==0) { run=0; timeout=10; } /* No data for 1 sec => quit */
        } else {
    ...
    if (quiet) {
        ...
        printf("First Packet Number: %lld\n", packet_0);
